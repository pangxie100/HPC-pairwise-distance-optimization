...
// packing + cache blocking + 8*6 register blocking with AVX2 + loop unrolling * 4
...
void pzyscale_C_k92(int M, int N, double beta, double *C, int LDC){
    int i, j;
    for (i = 0; i < M; i++){
        for (j = 0; j < N; j++){
            C(i,j) *= beta; 
...
void pzydgemm_cpu_opt_k92(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    int i, j, k;
    if (beta != 1.0) pzyscale_C_k92(M,N,beta,C,LDC);
    for (i = 0; i < M; i++){
        for (j = 0; j < N; j++){
...
// changed compared to kernel 8 and 9
#define KERNEL_K1_8x6_avx2_intrinsics_packing\
    ...
    b00 = _mm256_broadcast_sd(ptr_packing_b + 2);\
    b01 = _mm256_broadcast_sd(ptr_packing_b + 3);\
    ...
    b00 = _mm256_broadcast_sd(ptr_packing_b + 4);\
    b01 = _mm256_broadcast_sd(ptr_packing_b + 5);\
...
#define macro_kernel_8xkx6_avx2_packing\
    ...
    for (k = inner_k_count; k < inner_k_count + K4;){\
        KERNEL_K1_8x6_avx2_intrinsics_packing\
        KERNEL_K1_8x6_avx2_intrinsics_packing\
        KERNEL_K1_8x6_avx2_intrinsics_packing\
        KERNEL_K1_8x6_avx2_intrinsics_packing\
    }\
...
void pzypacking_b_k92(double *packsrc, double *packdst, int LDB, int dim_k, int dim_n){
    // kernel A * B is 8 * 4, thus we need four pointers to store each number in one row (col-major)
    double *src1, *src2, *src3, *src4, *src5, *src6, *dst;
    dst = packdst;
    int k, n;
    for (n = 0; n < dim_n; n += 6){
        src1 = packsrc + n * LDB;
...
void pzypacking_a_k92(double *packsrc, double *packdst, int LDA, int dim_m, int dim_k){
...
void pzydgemm_cpu_v92(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    if (beta != 1.0) pzyscale_C_k92(M, N, beta, C, LDC);
    ...
    for (n_count = 0; n_count < N; n_count += n_inc){
        ...
        for (k_count = 0; k_count < K; k_count += k_inc){
                ...
                pzypacking_b_k92(B + k_count + n_count * LDB, b_buffer, LDB, k_inc, n_inc);

            for (m_count = 0; m_count < M; m_count += m_inc){
                //printf("m_count = %d\n", m_count);
                m_inc = M - m_count > M_BLOCKING ? M_BLOCKING : M - m_count;
                pzypacking_a_k92(A + m_count + k_count * LDA, a_buffer, LDA, m_inc, k_inc);
                ...
                for (inner_m_count = 0; inner_m_count < M8; inner_m_count += 8){
                    ...
                    for (inner_n_count = 0; inner_n_count < N6; inner_n_count += 6){ 
                        ...
                        // add register reuse on C(i,j) to reduce the times of accessing C(i,j) in memory
                        macro_kernel_8xkx6_avx2_packing
                        /*
                        // print matrix
                        printf("matrix A:\n");
                        for (int m = 0; m < M; m++){
                ...
                if (M8 != m_inc) {
                    ...
                    pzydgemm_cpu_opt_k92(m_inc - M8, n_inc, k_inc, alpha, &A(M8, 0), LDA, B, LDB, 1.0, &C(M8, 0), LDC); // A+M8 move to M8 row, because it's column major
                }
                if (N6 != n_inc) {
                    printf("enter edge case for n_inc. \n");
                    pzydgemm_cpu_opt_k92(M8, n_inc - N6, k_inc, alpha, A, LDA, &B(0, N6), LDB, 1.0, &C(0, N6), LDC);
                }
            }
            //printf("m_count = %d\n", m_count);
            ...
            if (m_inc != M_BLOCKING && m_count != 0){
                ...
                pzydgemm_cpu_opt_k92(m_inc, N_BLOCKING, K, alpha, &A(m_count, 0), LDA, &B(0, n_count), LDB, 1.0, &C(m_count, n_count), LDC);
            }
        ...
        if (n_inc != N_BLOCKING && n_count != 0){
            ...
            pzydgemm_cpu_opt_k92(M, n_inc, K, alpha, A, LDA, &B(0, n_count), LDB, 1.0, &C(0, n_count), LDC);
        }
    }
