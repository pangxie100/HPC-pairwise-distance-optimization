...
void packing_a_k30(double *src, double *dst, int leading_dim, int dim_first, int dim_second){
...
void packing_b_k30(double *src,double *dst,int leading_dim,int dim_first,int dim_second){
...
void kernel_n_8_k30(double *a_buffer,double *b_buffer,double *c_ptr,int m,int K,int LDC,double alpha){
...
void kernel_n_4_k30(double *a_buffer,double *b_buffer,double *c_ptr,int m,int K,int LDC,double alpha){
    int m_count,m_count_sub;
    int i,j,k;
    double *C=c_ptr;
    double sc0,sc1,sc2,sc3,sc4,sc5,sc6,sc7,sa,sb0,sb1,sb2,sb3,sb4,sb5,sb6,sb7;
...
void macro_kernel_k30(double *a_buffer,double *b_buffer,int m,int n,int k,double *C, int LDC,double alpha){
    ...
    for (n_count_sub=n,n_count=0;n_count_sub>7;n_count_sub-=8,n_count+=8){
        ...
        kernel_n_8_k30(a_buffer,b_buffer+n_count*k,C+n_count*LDC,m,k,LDC,alpha);
    }
    for (;n_count_sub>3;n_count_sub-=4,n_count+=4){
        //call the m layer with n=4
        kernel_n_4_k30(a_buffer,b_buffer+n_count*k,C+n_count*LDC,m,k,LDC,alpha);
    }
...
void mydgemm_cpu_v30(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    
    if (beta != 1.0) scale_c_k30(C,M,N,LDC,beta);
