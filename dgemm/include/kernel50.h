...
void scale_c_k50(double *C,int M, int N, int LDC, double scalar){
    int i,j;
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            C(i,j)*=scalar;
...
void packing_b_24x8_version1_k50(double *src,double *dst,int leading_dim,int dim_first,int dim_second){
...
void packing_a_24x8_k50(double *src, double *dst, int leading_dim, int dim_first, int dim_second){
...
{
    int i,j,k;
    if (beta != 1.0) scale_c_k50(C,M,N,LDC,beta);
    int M4,N4,K4;
    ...
    for (k_count=0;k_count<K;k_count+=k_inc){
        ...
        for (n_count=0;n_count<N_MAIN;n_count+=N_BLOCKING){
            packing_b_24x8_version1_k50(B+k_count+n_count*LDB,b_buffer,LDB,k_inc,N_BLOCKING);
            for (m_count=0;m_count<M_MAIN;m_count+=M_BLOCKING){
                packing_a_24x8_k50(A+m_count+k_count*LDA,a_buffer,LDA,M_BLOCKING,k_inc);
