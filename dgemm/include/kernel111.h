...
// discontinuous packing + cache blocking + 16 * 14 register blocking with AVX512 + loop unrolling * 4
...
void pzyscale_C_k111(int M, int N, double beta, double *C, int LDC){
    int i, j;
    for (i = 0; i < M; i++){
        for (j = 0; j < N; j++){
            C(i,j) *= beta; 
...
void pzydgemm_cpu_opt_k111(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    int i, j, k;
    if (beta != 1.0) pzyscale_C_k111(M, N, beta, C, LDC);
    for (i = 0; i < M; i++){
        for (j = 0; j < N; j++){
...
#define KERNEL_K1_16x14_avx512_intrinsics_packing_v2\
    ...
    b00 = _mm512_set1_pd(*(ptr_packing_b4));\
    b01 = _mm512_set1_pd(*(ptr_packing_b4 + 1));\
    ...
    b00 = _mm512_set1_pd(*(ptr_packing_b5));\
    b01 = _mm512_set1_pd(*(ptr_packing_b5 + 1));\
    ...
    b00 = _mm512_set1_pd(*(ptr_packing_b6));\
    b01 = _mm512_set1_pd(*(ptr_packing_b6 + 1));\
...
// test and debug
// printf("cx0[0] = %5.2f, cy0[0] = %5.2f, cz0[0] = %5.2f\n", cx0[0], cy0[0], cz0[0]);
#define macro_kernel_16xkx14_avx512_packing_v2\
    ...
    for (k = inner_k_count; k < inner_k_count + K4;){\
        KERNEL_K1_16x14_avx512_intrinsics_packing_v2\
        KERNEL_K1_16x14_avx512_intrinsics_packing_v2\
        KERNEL_K1_16x14_avx512_intrinsics_packing_v2\
        KERNEL_K1_16x14_avx512_intrinsics_packing_v2\
    }\
...
void pzypacking_b_k111(double *packsrc, double *packdst, int LDB, int dim_k, int dim_n){
...
void pzypacking_a_k111(double *packsrc, double *packdst, int LDA, int dim_m, int dim_k){
...
// changed vs kernel 101
void pzydgemm_cpu_v111(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    if (beta != 1.0) pzyscale_C_k111(M, N, beta, C, LDC);
    ...
    double *ptr_packing_a, *ptr_packing_b0, *ptr_packing_b1, *ptr_packing_b2, *ptr_packing_b3, *ptr_packing_b4, *ptr_packing_b5, *ptr_packing_b6;
    ...
    for (n_count = 0; n_count < N; n_count += n_inc){
        ...
        for (k_count = 0; k_count < K; k_count += k_inc){
                ...
                pzypacking_b_k111(B + k_count + n_count * LDB, b_buffer, LDB, k_inc, n_inc);

            for (m_count = 0; m_count < M; m_count += m_inc){
                //printf("m_count = %d\n", m_count);
                m_inc = M - m_count > M_BLOCKING ? M_BLOCKING : M - m_count;
                pzypacking_a_k111(A + m_count + k_count * LDA, a_buffer, LDA, m_inc, k_inc);
                ...
                for (inner_m_count = 0; inner_m_count < M16; inner_m_count += 16){
                    ...
                    for (inner_n_count = 0; inner_n_count < N14; inner_n_count += 14){ 
                        ...
                        */
                        ...
                        ptr_packing_b4 = ptr_packing_b3 + k_inc * 2;
                        ptr_packing_b5 = ptr_packing_b4 + k_inc * 2;
                        ptr_packing_b6 = ptr_packing_b5 + k_inc * 2;

                        // add register reuse on C(i,j) to reduce the times of accessing C(i,j) in memory
                        macro_kernel_16xkx14_avx512_packing_v2
                        /*
                        // print matrix
                        printf("matrix A:\n");
                        for (int m = 0; m < M; m++){
                ...
                if (M16 != m_inc) {
                    ...
                    pzydgemm_cpu_opt_k111(m_inc - M16, n_inc, k_inc, alpha, &A(M16, 0), LDA, B, LDB, 1.0, &C(M16, 0), LDC); // A+M16 move to M16 row, because it's column major
                }
                if (N14 != n_inc) {
                    printf("enter edge case for n_inc. \n");
                    pzydgemm_cpu_opt_k111(M16, n_inc - N14, k_inc, alpha, A, LDA, &B(0, N14), LDB, 1.0, &C(0, N14), LDC);
                }
            }
            //printf("m_count = %d\n", m_count);
            ...
            if (m_inc != M_BLOCKING && m_count != 0){
                ...
                pzydgemm_cpu_opt_k111(m_inc, N_BLOCKING, K, alpha, &A(m_count, 0), LDA, &B(0, n_count), LDB, 1.0, &C(m_count, n_count), LDC);
            }
        ...
        if (n_inc != N_BLOCKING && n_count != 0){
            ...
            pzydgemm_cpu_opt_k111(M, n_inc, K, alpha, A, LDA, &B(0, n_count), LDB, 1.0, &C(0, n_count), LDC);
        }
    }
