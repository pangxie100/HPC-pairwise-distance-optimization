...
// kernel 9 new version from Optimizing-DGEMM-on-Intel-CPUs-with-AVX512F

void scale_c_k91(double *C,int M, int N, int LDC, double scalar){
    int i,j;
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            C(i,j)*=scalar;
...
void packing_a_k91(double *src, double *dst, int leading_dim, int dim_first, int dim_second){
...
void packing_b_k91(double *src,double *dst,int leading_dim,int dim_first,int dim_second){
...
void mydgemm_cpu_opt_k91(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    int i,j,k;
    if (beta != 1.0) scale_c_k91(C,M,N,LDC,beta);
    for (i=0;i<M;i++){
        for (j=0;j<N;j++){
            double tmp=C(i,j);
            for (k=0;k<K;k++){
...
#define macro_kernel_4xkx4_packing\
    ...
    _mm256_storeu_pd(&C(i,j+2), _mm256_add_pd(c20,_mm256_loadu_pd(&C(i,j+2))));\
    _mm256_storeu_pd(&C(i,j+3), _mm256_add_pd(c30,_mm256_loadu_pd(&C(i,j+3))));
void macro_kernel_gemm_k91(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double *C, int LDC){
    int i,j,k;
...
void mydgemm_cpu_v91(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double beta, double *C, int LDC){
    ...
    for (n_count=0;n_count<N;n_count+=n_inc){
        ...
        for (k_count=0;k_count<K;k_count+=k_inc){
            ...
            packing_b_k91(B+k_count+n_count*LDB,b_buffer,LDB,k_inc,n_inc);
            for (m_count=0;m_count<M;m_count+=m_inc){
                //m_inc = (M-m_count>M_BLOCKING)?M_BLOCKING:N-m_count;
                m_inc = (M-m_count>M_BLOCKING)?M_BLOCKING:M-m_count;
                packing_a_k91(A+m_count+k_count*LDA,a_buffer,LDA,m_inc,k_inc);
                //macro kernel: to compute C += A_tilt * B_tilt
                macro_kernel_gemm_k91(m_inc,n_inc,k_inc,alpha,a_buffer, LDA, b_buffer, LDB, &C(m_count, n_count), LDC);
            }
        }
    }
